...
const fecthing = async () => {
  try {
    const { data } = await useFetch(
      `${import.meta.env.VITE_APP_API_URL_MEGATRON}/clients/${route.params.clinets}`,
      {
        headers: {
...
const submit = async () => {
  ...
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      const { data } = await useFetch(
        `${import.meta.env.VITE_APP_API_URL_MEGATRON}/clients/${route.params.clinets}?_method=PUT`,
        {
          method: "POST",
          body: formDataToSend,
          headers: {
