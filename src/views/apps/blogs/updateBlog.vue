  ...
  <div v-if="!loading">
    ...
    >
      ...
      <button
        ...
        type="submit"
      >
        <span v-if="!loading">{{ $t("updateBlog") }}</span>
...
const fetchingV2 = async () => {
  const { data } = await useFetch(
    `${import.meta.env.VITE_APP_API_URL_NEW}/blogs/${route.params.blog}`,
    {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("authToken")}`,
      },
  ...
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_APP_API_URL_NEW}/blogs/${route.params.blog}?_method=PUT`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
      ...
      if (response.ok && data?.status) {
        ...
        router.push({ name: "apps-blog-all" });
      } else {
