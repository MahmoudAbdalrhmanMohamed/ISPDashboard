<template>
  <div class="card">
    ...
    <div class="card-body pt-0">
      ...
      >
        <!-- First Name Column -->
        <template v-slot:firstName="{ row: contact }">{{
          contact.firstName
        }}</template>
        <!-- Last Name Column -->
        <template v-slot:lastName="{ row: contact }">{{
          contact.lastName
        }}</template>
        <!-- Phone Column -->
        ...
        <!-- Message Column -->
        <template v-slot:message="{ row: contact }">{{
          contact.message
        }}</template>
        <!-- Categories Column -->
        <template v-slot:categories="{ row: contact }">{{
          ...
          >
            <div class="menu-item px-3">
              <router-link
                ...
                :to="`/apps/contacts/updateContact/${contact.id}`"
              >
                {{ $t("edit") }}
              </router-link>
            </div>
            ...
            <div class="menu-item px-3">
              <a @click="deleteContact(contact.id)" class="menu-link px-3">{{
                $t("delete")
              }}</a>
            </div>
          </div>
...
const fetching = async (page = 1) => {
  try {
    ...
    initContacts.value = [...tableData.value];
...
const deleteContact = async (id) => {
  ...
  if (confirmResult.isConfirmed) {
    try {
      await useFetch(`${import.meta.env.VITE_APP_API_URL_NEW}/contacts/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
      ...
      tableData.value = tableData.value.filter((contact) => contact.id !== id);
      initContacts.value = initContacts.value.filter(
...
const deleteSelectedContacts = async () => {
  ...
  const confirmResult = await Swal.fire({
    title: "Are you sure?",
    text: `You are about to delete ${selectedIds.value.length} contacts.`,
    icon: "warning",
    showCancelButton: true,
    ...
    try {
      await Promise.all(
        selectedIds.value.map((id) =>
          useFetch(`${import.meta.env.VITE_APP_API_URL_NEW}/contacts/${id}`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("authToken")}`,
            },
      ...
      Swal.fire({
        title: "Deleted!",
        text: "The selected contacts have been deleted.",
        icon: "success",
        confirmButtonText: "OK",
        customClass: { confirmButton: "btn btn-primary" },
      });
...
const debouncedSearch = debounce(() => {
  if (search.value) {
    ...
    tableData.value = initContacts.value.filter((item) =>
    ...
    tableData.value = [...initContacts.value];
  }
}, 300);
